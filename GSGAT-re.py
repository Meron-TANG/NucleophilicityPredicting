...
def load_csv(random_seed, db_file_path):
    df = pd.read_csv(db_file_path, index_col=0)
...
def load_train_valid_test(random_seed, db_file_path):
    df = pd.read_csv(db_file_path , index_col=0)
    df = df[['homo_smiles', 'homo_sol_smiles', 'N']]
    data = shuffle(df, random_state=random_seed)
...
data = load_csv(random_seed=seed, db_file_path)  # !!!!
