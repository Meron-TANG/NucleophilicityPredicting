...
# def egcn_collate_emodel(data):
#
#     # def maccs_index(threshold):
#     #
...
#     #         return col_corr
#     #
#     #     df = pd.read_csv('D:/Dataset/统一smile亲核试剂1100版本.csv', index_col=0)
#     #
#     #     df['com'] = df.SMILES + '.' + df.SOL
#     #
#     #     mol = [Chem.MolFromSmiles(i) for i in df['com']]
#     #
#     #     # mol = [Chem.MolFromSmiles(i) for i in data['com']]
#     #     fp1 = [MACCSkeys.GenMACCSKeys(i) for i in mol]
#     #
#     #     a = np.array(fp1, int)
#     #     b = pd.DataFrame(a)
#     #
#     #     return list(correlation(b, threshold))
#
#
#     # descriptor_names = ['ExactMolWt','HeavyAtomMolWt','MaxAbsPartialCharge', 'fr_Al_OH_noTert','fr_COO2','fr_C_O_noCOO','fr_Nhpyrrole', 'fr_benzene','fr_ketone_Topliss',\
#     # 'fr_nitrile','fr_nitro_arom_nonortho','fr_phenol','fr_phenol_noOrthoHbond','NumValenceElectrons','HeavyAtomCount']
#
#     egcn = ['RingCount','MolWt','']
#     #descriptor_names = ['BCUT2D_CHGLO','BCUT2D_LOGPHI','BertzCT','Chi0','Chi0n','Chi0v','Chi1','Chi1n','Chi1v','Chi2n','Chi2v','Chi3n','Chi3v','Chi4n','Chi4v','EState_VSA9',\
#     # 'ExactMolWt','FpDensityMorgan2','FpDensityMorgan3','HeavyAtomCount','HeavyAtomMolWt','LabuteASA','MaxAbsEStateIndex','MaxAbsPartialCharge','MolMR','NOCount','NumAliphaticRings','NumAromaticCarbocycles',\
#     # 'NumAromaticRings','NumHAcceptors','NumSaturatedRings','NumValenceElectrons','RingCount','SMR_VSA7','SlogP_VSA12','SlogP_VSA6','VSA_EState10','VSA_EState6','fr_Al_OH_noTert','fr_COO',\
#     # 'fr_COO2', 'fr_C_O_noCOO','fr_Nhpyrrole','fr_benzene','fr_ketone_Topliss','fr_nitrile','fr_nitro_arom_nonortho','fr_phenol','fr_phenol_noOrthoHbond']
#
...
#
#
#     # maccs_list = maccs_index(0.9)
#
#     # maccs_list  = [96, 33, 67, 70, 71, 40, 73, 103, 105, 143, 147, 117, 56, 58, 60, 61, 63]
#
#     def get_bit(smile):
#
...
#
# #        a = np.array(fp1, int)[maccs_list].reshape(1, -1)  # len = 17
#         a = np.array(fp1, int).reshape(1, -1)  # len = 167
#
#         # fp2 = pyAvalonTools.GetAvalonFP(mol,nBits=3096)
#         # fp3 = AllChem.GetMorganFingerprintAsBitVect(mol,2,nBits=1024)
#
#         # b = np.array(fp2,int).reshape(1,-1)
#         # c = np.array(fp3,int).reshape(1,-1)
#
#         # bit = np.concatenate([a,b,c],1)
#         # moe = np.array(descriptor_calculation.CalcDescriptors(mol)).reshape(1, -1)  # len = 49
#
#         # moe = np.nan_to_num(moe)
#         # scaler = MinMaxScaler()
#         # scaler.fit(moe)
#         # moe = scaler.fit_transform(moe)
#
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# testdata = load_data(test_data, 'test_data_load', True, 'N', 'com')
# testloader = DataLoader(testdata, batch_size=1, shuffle=True,collate_fn=collate_emodel)
...
if cross_vali:
    ...
    for _ in range(k_fold):
        ...
        x, y = k_fold_data(data, k_fold, _, 32)
        ...
        # test_score = load_model(stopper_filename,testloader,n_feats,e_feats)
    ...
    # out.to_csv('result/FGAT_macc_relu_kfold_result_{}_wholeseed_{}.csv'.format(estimate[0],seed))
    ...
    for _ in range(split_size):
        ...
        seed_spl = int(np.random.randint(1, 999, 1)) #每次随机一个数字 用作分割数据集的randam——state
    ...
    out.to_csv('result/rsp_result_{}_size_{}.csv'.format(estimate[0],split_size))
